...
.controller('AppCtrl', function($ionicHistory,$state,$scope, $ionicModal, $timeout,$http, $sessionStorage,$window,searchdate,$rootScope,$ionicLoading) {
  ...
  $scope.location_data = [];
  $http.get($scope.baseurl+"/getlocation").then((locationData)=>
  {
    $scope.location_data = locationData.data.data;
    console.log($scope.location_data);
  });
  //$scope.demolist = ['c','c++','java'];
  $http.get($scope.baseurl+"/get_all_degree").then((data) =>
  {
    //console.log(data.data);
    $scope.degreeListMain = data.data;
    for(i = 0; i< $scope.degreeListMain.length; i++)
    {
      var degree = $scope.degreeListMain[i].degree_name;
      var degree_id = $scope.degreeListMain[i].id;
      if($scope.degreeList.indexOf(degree) == -1)
      {
        $scope.degreeList.push(degree);
        $scope.degreeid.push(degree_id);
      }
    }
  });
  ...
  $http.get($scope.baseurl+"/get_all_speciality").then(function success(data)
  {
    $scope.spListMain = data.data;
    for(i = 0; i < $scope.spListMain.length; i++)
    {
      var degree = $scope.spListMain[i].specialty_name;
      var degree_id = $scope.spListMain[i].id;
      //console.log(degree);
      if($scope.splList.indexOf(degree) == -1)
      {
        $scope.splList.push(degree);
        $scope.spListid.push(degree_id);
      }
    }
  });
  $http.get($scope.baseurl+"/get_all_language").then(function success(data)
  {
    //$scope.langList = data.data;
    $scope.langListMain = data.data;
    for(i = 0; i< $scope.langListMain.length; i++)
    {
      var degree = $scope.langListMain[i].language_name;
      var degree_id = $scope.langListMain[i].id;
      if($scope.langList.indexOf(degree) == -1)
      {
        $scope.langList.push(degree);
        $scope.langListid.push(degree_id);
      }
    }
  });
...
/*if($sessionStorage.userSessionStatus != undefined)
{
  $sessionStorage.userSessionStatus = false;
  $scope.noSessionStatus = true;
}*/
if($sessionStorage.doctorlogin == true)
{
    ...
    $http.post($scope.baseurl+url,$scope.loginData).then(function mySucc(response)
  {
    ...
    if(data.message == "Successfully Logged in")
    {
      ...
      $timeout(function() {
        ...
        $scope.closeDocLogin();
        $scope.doctorlogin = true;
        $window.location.href = "#/app/doctor_profile";
      }, 1000);
    }
    else
    {
  ...
  $scope.docProfUpdate = function(data)
  {
    ...
    //console.log(doctor_degree);

    //console.log(doctor_languages);

    //console.log(doctor_speciality);
    ...
    //data.doctor_languages = undefined;
    ...
    data.location = (data.location != undefined)?data.location:"";
    ...
    if($sessionStorage.doctor_id != undefined)
    {
      ...
      }).then(function success(response)
      {
        ...
        $scope.docprof.doctor_degree = getStrArr(docprofile.doctor_degree, $scope.degreeList, $scope.degreeid);
        
        $scope.docprof.doctor_language = getStrArr(docprofile.doctor_languages, $scope.langList, $scope.langListid);
        
        $scope.docprof.doctor_speciality = getStrArr(docprofile.specialty, $scope.splList, $scope.spListid);
        
        docprofile.doctor_firstname = data.firstname;
        
        docprofile.doctor_lastname = data.lastname;
        
        docprofile.about_doctor = data.about_doctor;
        
        docprofile.doctor_office_country =  data.doctor_office_country;
        
        docprofile.doctor_office_address = data.doctor_office_address;
        
        docprofile.doctor_office_state = data.doctor_office_state;
        
        docprofile.doctor_office_city = data.doctor_office_city;
        
        docprofile.doctor_office_zip = data.doctor_office_zip;

        var new_data = docprofile1.data[0].docprofile;

        $sessionStorage.userSession = "";
        */

        //$sessionStorage.userSession = JSON.stringify(new_data);
  ...
  $scope.doLogin = function() {
    ...
    $scope.doDocSignup = function()
    {
      ...
      //console.log(get_url($scope.signUpData));
      $http.post($scope.baseurl+url, $scope.signUpData).then(function mySucces(response) {
        ...
        if(response.data.message == "Successfully Submitted")
        {
         ...
         {
          $ionicLoading.hide();
          $scope.signupError = response.data.status.message;
...
.controller('BMDCtrl', function($ionicModal,$rootScope,$scope, $stateParams,$http,speciality,location,insurance,searchdate,$window,$timeout,$filter,$ionicLoading, $sessionStorage) {
